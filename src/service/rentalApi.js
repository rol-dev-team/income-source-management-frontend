...
export const deleteRentalPostingSoftDelete = async (id) => {
  const response = await apiClient.post(`/rental/postings/${id}/soft-delete`);
  return response.data;
};

export const restoreRentalPosting = async (id) => {
  const response = await apiClient.put(`/rental/postings/${id}/restore`);
  return response.data;
};
