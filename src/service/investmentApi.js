...
export const getInvestmentLedgerSummary = (currentPage, pageSize, filter) =>
  apiClient.get("/investment/ledger-summary", {
    params: { page: currentPage, pageSize, filter },
  });



export const deleteInvestmentPostingSoftDelete = async (id) => {
  const response = await apiClient.post(`/investment/postings/${id}/soft-delete`);
  return response.data;
};

export const restoreInvestmentPosting = async (id) => {
  const response = await apiClient.put(`/investment/postings/${id}/restore`);
  return response.data;
};