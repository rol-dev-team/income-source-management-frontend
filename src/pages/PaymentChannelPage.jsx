...
import { paymentChannelFormFields } from "../constants/forms/paymentChannelForm";
import { paymentChannelFormValidationSchema } from "./../schemas/paymentChannelSchemas";
...
const PaymentChannelPage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const res = await fetchPaymentChannel(currentPage, pageSize);
        setData(res.data);
  ...
  const handleSubmit = async (formData) => {
    try {
      setIsLoading(true);
      if (selectedUser) {
        await updatePaymentChannel(selectedUser.id, formData);
        showToast.success(
    ...
    <div className='md:flex gap-3'>
      ...
      {(visibleSection === "both" || visibleSection === "table") && (
        <div
          ...
          <h3 className='text-lg font-semibold mb-3'>Payment Mode List</h3>
          {/* Top-right Button */}
      ...
      {(visibleSection === "both" || visibleSection === "form") && (
        <div
          ...
          <h3 className='text-lg font-semibold mb-3'>
            {selectedUser ? "Edit Payment Mode" : "Create Payment Mode"}
          </h3>
          <DynamicForm
            key={formKey}
            fields={paymentChannelFormFields}
            onSubmit={handleSubmit}
            validationSchema={paymentChannelFormValidationSchema}
