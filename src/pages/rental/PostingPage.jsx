...
// import { canChangeStatus, canEdit, capitalizeFirstLetter } from '../../helper/utility';
// import { AuthContext } from '../../App';

// import { getRentalPostingsPaginated, updateRentalPostingStatus } from '../../service/rentalApi';
...
//       setLoading(true);
//       try {
//         const res = await getRentalPostingsPaginated(currentPage, pageSize, filterStatus);
//         setData(res.data);
...
//       try {
//         const response = await updateRentalPostingStatus(selectedPosting.id, values);
//         showToast.success(response?.message || 'Posting status updated successfully!');
...
//     {
//       header: 'Entry Type',
//       accessor: (row) => capitalizeFirstLetter(row.entry_type),
//       sortable: true,
//     },

//     { header: 'Rent Received', accessor: 'rent_received', sortable: true },
//     { header: 'Party', accessor: 'party_name', sortable: true },
//     { header: 'House Name', accessor: 'house_name', sortable: true },
...
//     { header: 'Other Cost', accessor: 'other_cost_bdt', sortable: true },
//     { header: 'Security Money', accessor: 'security_money', sortable: true },
//     { header: 'Auto Adjust', accessor: 'auto_adjustment', sortable: true },
//     {
//       header: 'Refund Security',
...
//     navigate(`/rental/postings-create`, { state: { currency } });
//   };
...
import { AuthContext } from '../../App';
import { 
  getRentalPostingsPaginated, 
  updateRentalPostingStatus,
  deleteRentalPostingSoftDelete,
  restoreRentalPosting
...
const PostingPage = ({ isDeleteRestoreMode = false, defaultFilterStatus = "" }) => {
  ...
  const handleDelete = async (postingId) => {
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        setLoading(true);
        const response = await deleteRentalPostingSoftDelete(postingId);
  ...
  const handleRestore = async (postingId) => {
    if (window.confirm("Are you sure you want to restore this record?")) {
      try {
        setLoading(true);
        const response = await restoreRentalPosting(postingId);
  ...
  return (
    <div>
      <DataTable
        ...
        handleFilterChangeStatus={handleFilterChangeStatus}
        title="Rental Postings"
        link={isDeleteRestoreMode ? null : "/rental/postings-create"}
