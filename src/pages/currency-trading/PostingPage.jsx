...
//       setLoading(true);
//       try {
//         const res = await getCurrencyPostingsPaginated(
//           currentPage,
//           pageSize,
//           filterStatus
//         );
...
//       try {
//         const response = await updateCurrencyPostingStatus(
//           selectedPosting.id,
//           values
//         );
//         showToast.success(
...
//     {
//       header: "Party A/C No.",
//       accessor: "party_account_number",
//       sortable: true,
//     },
//     { header: "Currency Amount", accessor: "currency_amount", sortable: true },
...
//     navigate(`/currency-trading/create-posting`, { state: { currency } });
//   };
...
//         link='/currency-trading/create-posting'
//       />
...
import {
  ...
  getCurrencyPostingsPaginated,
  updateCurrencyPostingStatus,
  deleteCurrencyPosting,
  restoreCurrencyPosting, // Add this function
} from "../../service/currency-trading/currencyApi";
import { showToast } from "../../helper/toastMessage";
import {
  canChangeStatus,
...
const PostingPage = ({ isDeleteRestoreMode = false, defaultFilterStatus = "" }) => {
  ...
  const handleDelete = async (postingId) => {
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        setLoading(true);
        const response = await deleteCurrencyPosting(postingId);
  ...
  const handleRestore = async (postingId) => {
    if (window.confirm("Are you sure you want to restore this record?")) {
      try {
        setLoading(true);
        const response = await restoreCurrencyPosting(postingId);
  ...
  return (
    <div>
      <DataTable
        ...
        handleFilterChangeStatus={handleFilterChangeStatus}
        title="Currency Postings"
        link={isDeleteRestoreMode ? null : "/currency-trading/create-posting"}
