...
import { expenseFormFields } from "../constants/forms/expenseForm";
import { expenseFormValidationSchema } from "../schemas/expenseTypeSchemas";
import {
  createExpenseTypes,
  fetchExpenseTypes,
  updateExpenseTypes,
} from "./../service/expenseTypeApi";
...
const ExpenseTypePage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const res = await fetchExpenseTypes(currentPage, pageSize);
        setData(res.data);
  ...
  const dynamicExpenseFormFieldsFormFields = useMemo(() => {
    const fields = JSON.parse(JSON.stringify(expenseFormFields));
  ...
  const columns = [
    ...
    { header: "Type Name", accessor: "expense_type", sortable: true },
    {
      header: "Actions",
      accessor: (row) => (
        <button
  ...
  const handleSubmit = async (formData) => {
    try {
      setIsLoading(true);
      if (selectedUser) {
        await updateExpenseTypes(selectedUser.id, formData);
        showToast.success(
          messages.user.updateSuccess || "Updated successfully!"
        );
      } else {
        await createExpenseTypes(formData);
    ...
    <div className='md:flex gap-3'>
      ...
      {(visibleSection === "both" || visibleSection === "table") && (
        <div
          ...
          <h3 className='text-lg font-semibold mb-3'>Income / Expense Types</h3>
          {/* Top-right Button */}
      ...
      {(visibleSection === "both" || visibleSection === "form") && (
        <div
          ...
          <h3 className='text-lg font-semibold mb-3'>
            {selectedUser ? "Edit Income / Expense Type" : "Create Income / Expense Type"}
          </h3>
          <DynamicForm
            key={formKey}
            fields={dynamicExpenseFormFieldsFormFields}
