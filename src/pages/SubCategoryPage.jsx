...
import { sourceSubCategoryFormValidationSchema } from "../schemas/sourceSubCategorySchemas";
import {
  createSourceSubCategory,
  fetchSourceSubCategory,
  updateSourceSubCategory,
} from "../service/sourceSubCategory";

import { sourceSubCategoryFormFields } from "./../constants/forms/sourceSubCategoryForm";
...
const SubCategoryPage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const res = await fetchSourceSubCategory(currentPage, pageSize);
        setData(res.data);
  ...
  const dynamicSourceSubCategoryFormFields = useMemo(() => {
    const fields = JSON.parse(JSON.stringify(sourceSubCategoryFormFields));
  ...
  const columns = [
    ...
    { header: "Source SubCategory", accessor: "subcat_name", sortable: true },
    {
      header: "Actions",
      accessor: (row) => (
        <button
  ...
  const handleSubmit = async (formData) => {
    try {
      setIsLoading(true);
      if (selectedUser) {
        await updateSourceSubCategory(selectedUser.id, formData);
        showToast.success(
          messages.user.updateSuccess || "Updated successfully!"
        );
      } else {
        await createSourceSubCategory(formData);
    ...
    <div className='md:flex gap-3'>
      ...
      {(visibleSection === "both" || visibleSection === "table") && (
        <div
          ...
          <h3 className='text-lg font-semibold mb-3'>Sub-Category List</h3>
          {/* Top-right Button */}
      ...
      {(visibleSection === "both" || visibleSection === "form") && (
        <div
          ...
          <h3 className='text-lg font-semibold mb-3'>
            {selectedUser ? "Edit Sub-Category" : "Create Sub-Category"}
          </h3>
          <DynamicForm
            key={formKey}
            fields={dynamicSourceSubCategoryFormFields}
            onSubmit={handleSubmit}
            validationSchema={sourceSubCategoryFormValidationSchema}
