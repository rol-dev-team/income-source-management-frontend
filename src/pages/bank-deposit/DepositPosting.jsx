...
import {
  getBankDepositPostingsPaginated,
  updateBankDepositPostingStatus,
  deleteBankDepositPostingSoftDelete,
  restoreBankDepositPosting,
} from "../../service/bankDepositApi";

const DepositPosting = ({ isDeleteRestoreMode = false, defaultFilterStatus = "" }) => {
  const { user } = useContext(AuthContext) || {};
  const [data, setData] = useState([]);
  const [totalPages, setTotalPages] = useState(1);
  const [currentPage, setCurrentPage] = useState(1);
  ...
  const handleDelete = async (postingId) => {
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        setLoading(true);
        const response = await deleteBankDepositPostingSoftDelete(postingId);
  ...
  const handleRestore = async (postingId) => {
    if (window.confirm("Are you sure you want to restore this record?")) {
      try {
        setLoading(true);
        const response = await restoreBankDepositPosting(postingId);
  ...
  return (
    <div>
      <DataTable
        ...
        handleFilterChangeStatus={handleFilterChangeStatus}
        title='Bank Deposit List'
        link={isDeleteRestoreMode ? null : '/bank-deposit-create'}
