...
const PaymentChannelDetails = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const res = await fetchPaymentChannelDetails(currentPage, pageSize);
        setData(res.data);
  ...
  useEffect(() => {
    const loadChannelOptions = async () => {
      try {
        const { data } = await fetchPaymentChannel();
  ...
  const dynamicPaymentChannelDetailsFormFields = useMemo(() => {
    const fields = JSON.parse(JSON.stringify(paymentChannelDetailsFormFields));

    const channelIdField = fields.find((field) => field.name === "channel_id");
    if (channelIdField) {
  ...
  const handleSubmit = async (formData) => {
    try {
      setIsLoading(true);
      if (selectedUser) {
        await updatePaymentChannelDetails(selectedUser.id, formData);
        showToast.success(
          messages.user.updateSuccess || "Updated successfully!"
        );
      } else {
        await createPaymentChannelDetails(formData);
    ...
    <div className='md:flex gap-3'>
      ...
      {(visibleSection === "both" || visibleSection === "table") && (
        <div
          ...
          <h3 className='text-lg font-semibold mb-3'>Payment Channel</h3>
          {/* Top-right Button */}
      ...
      {(visibleSection === "both" || visibleSection === "form") && (
        <div
          ...
          <h3 className='text-lg font-semibold mb-3'>
            {selectedUser ? "Edit Payment Channel" : "Create Payment Channel"}
          </h3>
          <DynamicForm
            key={formKey}
            fields={dynamicPaymentChannelDetailsFormFields}
