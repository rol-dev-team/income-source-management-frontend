...
import {
  ChevronRightIcon,
  ChevronLeftIcon,
  PencilSquareIcon, // You can keep this if needed elsewhere, though not used in the snippet
...
} from "../service/transactionTypeApi";
import { transactionFormFields } from "../constants/forms/transactionForm";
import { transactionFormValidationSchema } from "../schemas/transactionSchemas";
...
const TransactionTypePage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const res = await fetchTransactionTypes(currentPage, pageSize);
        setData(res.data);
  ...
  const handleSubmit = async (formData) => {
    try {
      setIsLoading(true);
      if (selectedUser) {
        await updateTransactionTypes(selectedUser.id, formData);
        showToast.success(
          messages.user.updateSuccess || "Updated successfully!"
        );
      } else {
        await createTransactionTypes(formData);
    ...
    <div className='md:flex gap-3'>
      ...
      {(visibleSection === "both" || visibleSection === "table") && (
        <div
          ...
          <h3 className='text-lg font-semibold mb-3'>Transaction Types List</h3>
          {/* Top-right Button */}
      ...
      {(visibleSection === "both" || visibleSection === "form") && (
        <div
          ...
          <h3 className='text-lg font-semibold mb-3'>
            {selectedUser ? "Edit Transaction Type" : "Create Transaction Type"}
          </h3>
          <DynamicForm
            key={formKey}
