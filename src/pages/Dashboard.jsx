...
import { fetchForignCurrencyByPaymentChannel } from "../service/paymentChannelDetailsApi";
...
const FinancialDashboard = () => {
  ...
  useEffect(() => {
    const fetchCurrencyMatrix = async () => {
      try {
        const res = await fetchForignCurrencyByPaymentChannel();
        const payload = res ?? {};
        setCurrencyCodes(Object.values(payload.currencies ?? {}));
        setCurrencyMatrix(Array.isArray(payload.matrix) ? payload.matrix : []);
      } catch (err) {
        console.error(err);
        setCurrencyCodes([]);
        setCurrencyMatrix([]);
...
useEffect(() => {
  ...
  const currencyMatrixLookup = useMemo(() => {
    const lookup = {};

    currencyMatrix.forEach((row) => {
      const methodName = row?.method_name;
      if (!methodName) return;
      lookup[methodName] = row.balances || {};
      lookup[String(methodName).toLowerCase()] = row.balances || {};
    });
              ...
              <thead>
                <tr className='bg-gradient-to-r from-gray-50 to-gray-100'>
                  ...
                  {currencyCodes.map((code) => (
                    <th
                      key={code}
                      className='text-right py-4 px-6 text-xs font-bold text-gray-700 uppercase tracking-wider'>
                      {code}
                    </th>
                  ))}
                </tr>
                ...
                {accountBalance.filter((item)=>item.method_name !== "TOTAL").map((account, index) => {
                  ...
                  return (
                    <tr
                      ...
                      {currencyCodes.map((code) => (
                        <td key={code} className='py-5 px-6 text-right'>
                          <span className='font-semibold text-gray-900'>
                            {formatNumber(currencyBalances[code] ?? 0)}
                          </span>
                        </td>
                      ))}
                    </tr>
