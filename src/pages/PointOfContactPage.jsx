...
import {
  createPointsOfContact,
  fetchPointsOfContact,
  updatePointsOfContact,
} from "../service/pointOfContactApi";
import { pointOfContactFormFields } from "../constants/forms/pointOfContactForm";
import { pointOfContactFormValidationSchema } from "../schemas/pointOfContactSchemas";
...
const PointOfContactPage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const res = await fetchPointsOfContact(currentPage, pageSize);
        setData(res.data);
  ...
  const columns = [
    ...
    { header: "Name", accessor: "contact_name", sortable: true },
    { header: "Account No.", accessor: "contact_no", sortable: true },
  ...
  const handleSubmit = async (formData) => {
    try {
      setIsLoading(true);
      if (selectedUser) {
        await updatePointsOfContact(selectedUser.id, formData);
        showToast.success(
          messages.user.updateSuccess || "Updated successfully!"
        );
      } else {
        await createPointsOfContact(formData);
    ...
    <div className='md:flex gap-3'>
      ...
      {(visibleSection === "both" || visibleSection === "table") && (
        <div
          ...
          <h3 className='text-lg font-semibold mb-3'>Point of Contacts</h3>
          {/* Top-right Button */}
      ...
      {(visibleSection === "both" || visibleSection === "form") && (
        <div
          ...
          <h3 className='text-lg font-semibold mb-3'>
            {selectedUser ? "Edit Point of Contact" : "Create Point of Contact"}
          </h3>
          <DynamicForm
            key={formKey}
            fields={pointOfContactFormFields}
            onSubmit={handleSubmit}
            validationSchema={pointOfContactFormValidationSchema}
