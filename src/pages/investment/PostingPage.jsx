...
// import {
//   getInvestmentPostingsPaginated,
//   updateInvestmentPostingStatus,
...
//       setLoading(true);
//       try {
//         const res = await getInvestmentPostingsPaginated(
//           currentPage,
//           pageSize,
//           filterStatus
//         );
...
//       try {
//         const response = await updateInvestmentPostingStatus(
//           selectedPosting.id,
//           values
//         );
//         showToast.success(
...
//     navigate(`/investment/postings-create`, { state: { currency } });
//   };
...
//         link='/investment/postings-create'
//       />
...
import {
  getInvestmentPostingsPaginated,
  updateInvestmentPostingStatus,
  deleteInvestmentPostingSoftDelete,
  restoreInvestmentPosting,
} from "../../service/investmentApi";
...
const PostingPage = ({ isDeleteRestoreMode = false, defaultFilterStatus = "" }) => {
  ...
  const handleDelete = async (postingId) => {
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        setLoading(true);
        const response = await deleteInvestmentPostingSoftDelete(postingId);
  ...
  const handleRestore = async (postingId) => {
    if (window.confirm("Are you sure you want to restore this record?")) {
      try {
        setLoading(true);
        const response = await restoreInvestmentPosting(postingId);
  ...
  return (
    <div>
      <DataTable
        ...
        handleFilterChangeStatus={handleFilterChangeStatus}
        title='Investment Postings'
        link={isDeleteRestoreMode ? null : '/investment/postings-create'}
        isDeleteRestoreMode={isDeleteRestoreMode}
      />

      {/* Drawer - Only show if not in delete/restore mode */}
      {!isDeleteRestoreMode && (
        <>
          <div
            className={`fixed top-0 right-0 z-40 h-screen p-4 overflow-y-auto transition-transform ${
