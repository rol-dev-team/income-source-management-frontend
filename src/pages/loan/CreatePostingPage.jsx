...
const validationSchema = yup.object().shape({
  ...
  term_months: yup.number().when("transaction_type", {
    ...
    then: (schema) =>
      schema
        .required("Term in months is required for loan transactions")
        .min(0, "Term must be zero or positive"),
    otherwise: (schema) => schema.notRequired(),
  }),
  ...
  //   is: (val) => Number(val) > 0,  
  //   then: (schema) =>
  //     schema.required("Installment date is required when term greater than 0"),
  //   otherwise: (schema) => schema.notRequired(),
  // }),
installment_date: yup
  .number()
  .nullable(true) // Allow null if not required
  .when(
    // 1. Array of dependencies (the fields to check)
    ["transaction_type", "term_months"], 
    {
      // 2. The test function: Checks if BOTH conditions are met
      is: (transaction_type, term_months) => {
        const isLoanTransaction = ["loan_taken", "loan_given"].includes(transaction_type);
        const isTermValid = Number(term_months) > 0;
        
        // Return true only if BOTH are true
        return isLoanTransaction && isTermValid;
      },
      
      // 3. 'then' schema: Applied if the 'is' function returns true
      then: (schema) => 
        schema.required("Installment date is required as Term Months is greater than 0 for this loan transaction."),
      
      // 4. 'otherwise' schema: Applied if the 'is' function returns false
      otherwise: (schema) => 
        schema.notRequired(),
    }
  ),
...
const CreatePostingPage = () => {
  ...
  const handlePaymentChannelChange = (e) => {
    ...
    }else if(selectedPaymentChannelId == 15){
...
useEffect(() => {
    const term = formik.values.term_months;

    if (String(term) === "") {
      formik.setFieldValue("interest_rate", "");
    } 
    else if (Number(term) === 0) {
      formik.setFieldValue("interest_rate", 0);
    } 
    else {
      formik.setFieldValue("interest_rate", "");
    }
  }, [formik.values.term_months]);
            ...
            <div className='relative'>
              <select
                ...
                disabled={
                  ...
                  !!postingToEdit || formik.values.payment_channel_id == 15
                }
                className='mt-1 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5'>
                <option value='' disabled>
                  My A/C No.
              ...
              <>
                <div className='relative'>
                  <input
                    ...
                    onChange={formik.handleChange}
                    onBlur={formik.handleBlur}
                    disabled={Number(formik.values.term_months) === 0}
                    className='mt-1 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5'
                  />
